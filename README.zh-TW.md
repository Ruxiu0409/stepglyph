...
沒有背景錄影，也沒有雜亂的時間軸。Stepglyph 只會記錄 Codex 明確送到本機 recorder 的步驟。

### 真實截圖，不是重畫出來的 UI

每份 guide 都建立在實際 PNG 截圖上，marker、label、target、sensitive flag 和 export visibility 會以可編輯資料疊在上面。

### 用本機 Studio 把流程整理乾淨

在分享之前，先檢查流程、重寫步驟文字、移動標註、隱藏敏感步驟、刪掉雜訊，讓 guide 真的可以交付。

### 預設就能搭配 Codex

內建的 [Codex skill](packages/codex-skill/SKILL.md) 會給 Codex 一套清楚的 recording protocol，不需要改變你平常的聊天工作流。

### 匯出真的能交付的文件

同一份 captured project 可以產生 Markdown、HTML、JSON 和 annotated PNG assets，原始截圖仍然保留在本機檔案裡。
